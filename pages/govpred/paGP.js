...
export default function PennsylvaniaGovPredPage() {
  // Demo numbers—update as needed
  const demPercent = "55.2%";
  const gopPercent = "44.0%";
  const demChance = "82%";
  const predictedMargin = "Tilt D";
    ...
    <div className="min-h-screen bg-gray-900 text-white font-sans">
      ...
      <div className="bg-gray-800 py-10 px-6 text-center shadow-inner">
        <h1 className="text-4xl font-bold mb-4 leading-tight font-sans">
          Polidex Forecasts that <span className="text-[#577CCC]">Josh Shapiro</span>
          <br />is the favorite to Win the Pennsylvania Governor’s Race.
        </h1>
        <p className="text-lg font-sans">
          <span className="text-[#577CCC]">Democrats</span> have a{" "}
          <span className="text-[#577CCC]">{demChance}</span> Chance of Victory. Predicted Margin:{" "}
          <strong className="text-[#577CCC]">{predictedMargin}</strong>
        </p>
      ...
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row justify-center items-center gap-10 my-12 px-4">
        <CandidateCard
          name="Josh Shapiro"
          party="Democrat"
          percent={<span className="text-[#577CCC]">{demPercent}</span>}
          image="/shapiro.jpeg"
        />
        <div className="text-center text-gray-300">
          <p className="text-lg font-sans">Chance of Victory</p>
          <p className="text-2xl font-bold mt-1 text-[#577CCC] font-sans">
            Democrats {demChance}
          </p>
        </div>
        <CandidateCard
          name="Stacy Garrity"
          party="Republican"
          percent={<span className="text-red-500">{gopPercent}</span>}
          image="/garrity.jpg"
        />
      </div>
      ...
      <div className="bg-gray-800 p-6 rounded-lg shadow max-w-6xl mx-auto my-12">
        ...
        <div className="w-full flex items-center justify-center min-h-[380px] text-gray-300 text-lg">
          {/* Replace with <GovCountyMapPA /> if/when you build a county map */}
      ...
      <div className="text-center mb-12">
        ...
        >
          View Pennsylvania Governor Primaries →
        </Link>
      </div>
