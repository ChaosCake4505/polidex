...
export default function AlaskaSenPredPage() {
  return (
    <div className="min-h-screen bg-gray-900 text-white font-sans">
      ...
      <div className="bg-gray-800 py-10 px-6 text-center shadow-inner">
        <h1 className="text-4xl font-bold mb-4 leading-tight font-sans">
          Polidex Forecasts that <span className="text-[#FF5865]">Dan Sullivan</span>
          <br />is Favored to Win the Alaska Senate Race.
        </h1>
        <p className="text-lg font-sans">
          <span className="text-[#FF5865]">Republicans</span> have a{" "}
          <span className="text-[#FF5865]">68%</span> Chance of Victory. Predicted Margin:{" "}
          <strong className="text-[#FF5865]">Likely R</strong>
        </p>
        <p className="text-sm text-gray-400 italic mt-2 font-sans">
          * Alaska uses a top-four nonpartisan primary with ranked-choice voting in the general election.
        </p>
      </div>
      ...
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row justify-center items-center gap-10 my-12 px-4">
        <CandidateCard
          name="TBA"
          party="Democrat"
          percent={<span className="text-blue-400">44.1%</span>}
          image="/demlogo.png"
        />
        <div className="text-center text-gray-300">
          <p className="text-lg font-sans">Chance of Victory</p>
          <p className="text-2xl font-bold mt-1 text-[#FF5865] font-sans">Republicans 88%</p>
        </div>
        <CandidateCard
          name="Dan Sullivan"
          party="Republican"
          percent={<span className="text-[#FF5865]">55.8%</span>}
          image="/sullivan.jpg"
        />
      </div>

      {/* Statewide Map / Regions Placeholder */}
      <div className="bg-gray-800 p-6 rounded-lg shadow max-w-6xl mx-auto my-12">
        <h2 className="text-xl font-semibold mb-4 text-center">Statewide Forecast Map</h2>
        <div className="w-full flex items-center justify-center min-h-[380px] text-gray-300 text-lg">
          {/* Replace with <SenateRegionMapAK /> when ready */}
          Borough/Census Area forecast will appear here when available.
        </div>
      </div>
