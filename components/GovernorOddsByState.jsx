// components/GovernorOddsByState.jsx
import React from "react";

/**
 * rows: [{
 *   state, rep, dem,
 *   // Predicted vote share (0..1 or 0..100) — drives circle position
 *   repVote?: number, demVote?: number,
 *   // Odds of winning (0..1 or 0..100) — shows as "NN in 100" on the right
 *   repOdds?: number, demOdds?: number,
 *   // Optional party tags for color (defaults: rep=R, dem=D)
 *   repParty?: "R"|"D"|"I", demParty?: "R"|"D"|"I",
 * }]
 * highlightWidthPx?: number  // requested width; we DOUBLE it internally (same as Senate)
 */
export default function GovernorOddsByState({ rows = [], highlightWidthPx = 70 }) {
  const widthPx = 360; // 50–55–60 mirrored scale width
  const SCALE_MIN = 0.50;
  const SCALE_MAX = 0.60; // mirrored to left for D/Ind
  ...
  return (
    <div className="bg-gray-800/80 rounded-lg p-6 shadow max-w-5xl mx-auto my-10">
      <h3 className="text-center text-xl font-bold mb-4">Competitive Gubernatorial Races</h3>
      ...
      {/* Headers */}
      <div
        className="hidden sm:grid text-xs text-gray-400 px-2 pb-2
      ...
      <div className="divide-y divide-gray-700">
        {rows.map((r) => {
          ...
          // --- Vote shares (0..1) drive circle position ---
          ...
          const leaderShare = leaderIsRep ? repVote : demVote;

          // Position on mirrored 50→60 scale
          const leaderPosPx = posFromVote(leaderShare, leaderIsRep, centerX, halfW, SCALE_RANGE);

          // --- Odds (separate number) for the right column ---
          const repOdds = norm01(r.repOdds ?? r.repProb ?? 0);
          const demOdds = norm01(r.demOdds ?? r.demProb ?? 0);
          const leaderOdds100 = Math.round((leaderIsRep ? repOdds : demOdds) * 100);
          ...
          // (DOUBLE the requested width, same as your Senate component)
          const effectiveHighlight = (highlightWidthPx ?? 70) * 2;
          const halfBand = Math.max(8, Math.floor(effectiveHighlight / 2));
          const bandStart = leaderPosPx - halfBand;
          const bandWidth = halfBand * 2;

          // If band crosses 50, split at 50; else solid by side
          let bandBg = "rgba(96,165,250,0.22)"; // blue by default (left side)
          ...
          return (
            ...
            >
              ...
              <div className="relative h-10 overflow-visible">
                {/* Halo */}
                <div
                  className="absolute inset-y-3 left-1/2 -translate-x-1/2 pointer-events-none"
                  style={{ width: widthPx }}
                  aria-hidden="true"
                >
                  <div
